...
\\unrestrict GNe5kMfiI0ANWitVE3BZ0HVlF41EznPoORQML2l8w8Tg2gLdbf9IxWY2UPYdYuN`)
    ...
    it('should handle \\restrict command with chunking (regression test for #3705)', async () => {
...
\\unrestrict GNe5kMfiI0ANWitVE3BZ0HVlF41EznPoORQML2l8w8Tg2gLdbf9IxWY2UPYdYuN`,
      ...
      expect(errors).toEqual([])
      expect(value.tables['api_tokens']).toBeDefined()
      expect(value.tables['api_tokens']?.columns['id']).toBeDefined()
      expect(value.tables['api_tokens']?.columns['token']).toBeDefined()
    })
  })
