...
export const PublicSessionDetailPage: FC<Props> = async ({
  designSessionId,
}) => {
  ...
  const { data: timelineItems } = await supabase
    .from('timeline_items')
    .select('id, design_session_id, content, created_at, updated_at, building_schema_version_id, type, query_result_id, assistant_role')
    .eq('design_session_id', designSessionId)
  ...
  const { data: buildingSchemas } = await supabase
    ...
    .select('id, design_session_id, schema, created_at, git_sha, initial_schema_snapshot, schema_file_path')
    .eq('design_session_id', designSessionId)
  ...
  // Parse artifact data if available
  let parsedArtifact: Artifact | null = null
  if (artifactData?.artifact) {
    const artifactParseResult = safeParse(artifactSchema, artifactData.artifact)
    if (artifactParseResult.success) {
      parsedArtifact = artifactParseResult.output
    }
  }
  ...
  return (
    <SessionDetailPageClient
      ...
      isPublicView={true}
      initialArtifact={parsedArtifact}
    />
  )
}
