...
### CRITICAL: Standalone SQL Statements
- NEVER use standalone SQL statements (WITH, INSERT, UPDATE, DELETE) outside of pgTAP functions
- ALL SQL must be wrapped in pgTAP functions like lives_ok() or throws_ok()
- WRONG: WITH u AS (...) INSERT INTO ...; (standalone SQL with semicolon)
- RIGHT: SELECT lives_ok($$WITH u AS (...) INSERT INTO ...$$, 'description'); (wrapped in pgTAP function)
...
4. **Use Dollar Quoting Correctly**
   ...
   - CRITICAL: NEVER nest $$ inside $$ - this creates unterminated strings
   - WRONG: lives_ok($$SELECT * FROM func($$param$$)$$, 'desc')
   - RIGHT: lives_ok($$SELECT * FROM func('param')$$, 'desc')
   - Use single quotes inside $$ blocks, NOT nested $$
