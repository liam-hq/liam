...
pgTAP is a unit testing framework for PostgreSQL that outputs TAP (Test Anything Protocol) format.

Your mission: Generate pgTAP tests that validate schema design, constraints, business logic, and data integrity.
SUCCESS: Your tests catch schema issues early and enable automatic schema repair loops.
`
...
4. DO NOT include BEGIN/ROLLBACK - the system automatically wraps tests in transactions
5. One test case = One test type (INSERT, UPDATE, DELETE, or SELECT)
6. throws_ok takes ONLY 2 arguments: throws_ok(sql, error_code) - NO description!
7. Use gen_random_uuid() for UUIDs, NOT uuid_generate_v4()
8. In is() comparisons, cast integers to bigint: 5::bigint not 5
9. Keep setup steps minimal and focused
10. NEVER put semicolons inside pgTAP function calls - only after the closing parenthesis
`
...
### Schema Testing
- has_table(table_name) - Verify table exists
- has_column(table, column) - Verify column exists
- col_type_is(table, column, type) - Verify column data type
- has_pk(table) - Verify primary key exists
- has_fk(table) - Verify foreign key exists
...
❌ **DON'T include BEGIN/ROLLBACK**
   The system automatically wraps tests in transactions

❌ **DON'T test multiple operation types in one test**
   One test case = One type (INSERT, UPDATE, DELETE, or SELECT)

❌ **DON'T use throws_ok() with 3 arguments**
   WRONG: throws_ok($$...$$, '23503', 'description')
   RIGHT: throws_ok($$...$$, '23503')
   throws_ok takes ONLY 2 arguments!

❌ **DON'T use lives_ok() with 3 arguments**
   WRONG: lives_ok($$SELECT 1$$, 'result', 'description')
   RIGHT: lives_ok($$SELECT 1$$, 'description')

❌ **DON'T put semicolons inside pgTAP function calls**
   WRONG: lives_ok($$...$$, 'description';)
   RIGHT: lives_ok($$...$$, 'description');
   Semicolons go AFTER the closing parenthesis, not inside!

❌ **DON'T mix types in is()**
   WRONG: is((SELECT COUNT(*) FROM users), 5, 'desc')
   RIGHT: is((SELECT COUNT(*) FROM users), 5::bigint, 'desc')
   COUNT returns bigint, not integer!

❌ **DON'T use uuid_generate_v4()**
   WRONG: uuid_generate_v4() (requires uuid-ossp extension)
   RIGHT: gen_random_uuid() (built-in)

❌ **DON'T forget to count all assertions in plan()**
   plan(2) means exactly 2 test assertions will run

❌ **DON'T use hardcoded IDs without setup**
   Either setup the data or use subqueries to find IDs

❌ **DON'T create tests that depend on external state**
   Each test should be self-contained and setup its own data
`

const USAGE_GUIDE = `
## How to Use This Knowledge

1. **Read the test case requirements** (category, title, type)
2. **Choose the appropriate pgTAP functions** based on test type
3. **Reference the examples** that match your test type
4. **Write setup steps** if needed (using lives_ok)
5. **Write the main assertion** (lives_ok or throws_ok)
6. **Count your assertions** and set plan(count)
7. **Add finish()** at the end
8. **Use the saveTestcase tool** to save your generated test

Remember: Your tests enable automatic schema repair. Be precise about what should succeed and what should fail!
`
...
IMPORTANT:
- Use the type ({type}) to choose appropriate pgTAP functions
- Reference the examples for {type} operations in your knowledge
- Include plan() with the correct count
- Include finish() at the end
- DO NOT include BEGIN/ROLLBACK (system handles transactions)
- Use the saveTestcase tool to save your test code
`)
