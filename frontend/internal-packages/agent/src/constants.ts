 ...
 *
 * Supports up to 3 automatic test execution attempts:
 * - Initial execution: ~10 steps
 * - Retry cycles: 2 × 10 = 20 steps
 * - Total: 30 steps + buffer = 50 steps
 *
 * @see {@link routeAfterAnalyzeFailures} for retry limit logic (QA_AGENT_MAX_ATTEMPTS = 3)
 */
export const QA_AGENT_RECURSION_LIMIT = 50

/**
 * Maximum number of execution attempts for failed QA agent test cases.
 *
 * This represents the total number of execution attempts (initial + retries).
 *
 * Behavior:
 * - CI environment: 1 attempt (fast execution, no retries)
 * - Development/Production: 3 attempts (allows 2 retries after initial execution)
 *
 * @see {@link routeAfterAnalyzeFailures} for retry routing logic
 */
export const QA_AGENT_MAX_ATTEMPTS = process.env['CI'] === 'true' ? 1 : 3
