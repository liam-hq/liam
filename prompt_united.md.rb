...
# TODO: step1でテストが多くなりづらくなるようにラインカバレージの数値を確認させたい
...
puts <<~EOF
  ...
  **重要** テストファイルは、__tests__ディレクトリのようなものは新たに作らず、ソースファイルと同じディレクトリに.test.tsの拡張子で追加すること。
  ...
  もしあるなら、確認2を飛ばして、step2に進む。

  確認2:
  #{source_path} 内の 実装及びテストについて内容を確認する。
  実装やテストが複雑すぎて、ファイル分割したほうが良さそうか？

  分割したほうが良さそうな場合は、step2に進む。
  分割しなくてもokな場合は、before-step3に進む。

  </before-step2>

  <step2>
  #{source_path} 挙動を変えずに、ファイル分割を検討しつつ、既存のlintとtestが通るようにリファクタリングして。
